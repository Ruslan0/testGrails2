<%@ page import="reports.Assigned; reports.Report" %>
...
<div class="fieldcontain ${hasErrors(bean: reportInstance, field: 'currdate', 'error')} ">
  ...
  <g:datePicker name="currdate" precision="day" value="${reportInstance?.currdate}" default="none"
                noSelection="['': '']"/>
...
<div class="fieldcontain ${hasErrors(bean: reportInstance, field: 'hours', 'error')} required">
  <label for="hours">
    <g:message code="report.assigned.label" default="Assigned"/>
    <span class="required-indicator">*</span>
  </label>
  <g:select name="assigned"
            from="${reports.Assigned.list()}"
            value="${reportInstance?.assigned?.id}"
            optionValue="${{ r -> "${r.employer.login} - ${r.project.name} " }}"
            optionKey="id"
  />
